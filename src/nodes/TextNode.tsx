...
export const TextNode = ({
  ...
  const cursorPathRef = useRef<{
    startPath: number[]; startOffset: number;
    endPath: number[];   endOffset: number;
  } | null>(null);
  ...
  /** Save cursor/selection as DOM child-node indices + text offsets.
   *  Survives text-node normalization because it stores numeric paths
   *  instead of raw DOM references. Preserves non-collapsed selections. */
  const saveCursorPath = (): boolean => {
    ...
    const nodePath = (target: Node): number[] | null => {
      const p: number[] = [];
      ...
      while (n && n !== editor) {
        ...
        for (let i = 0; i < parent.childNodes.length; i += 1) {
          if (parent.childNodes[i] === n) { idx = i; break; }
        }
  ...
  const restoreCursorPath = (): Range | null => {
    ...
    const resolveNode = (path: number[]): Node | null => {
      let n: Node = editor;
      for (const index of path) {
        if (index >= n.childNodes.length) return null;
        n = n.childNodes[index];
      }
      return n;
    };
    const startNode = resolveNode(saved.startPath);
    const endNode   = resolveNode(saved.endPath);
  ...
  const getInlineStylesForCommand = (nextCommand: TextEditorCommand): Partial<CSSStyleDeclaration> | null => {
    ...
    if (nextCommand.type === "apply-block-style" && typeof nextCommand.blockStyle === "string") {
      const preset = getBlockStyleForCommand(nextCommand);
      ...
      if (typeof preset.italic === "boolean") {
        styles.fontStyle = preset.italic ? "italic" : "normal";
      }
      return styles;
    }
    return null;
  ...
  const applyFormatCommandToSelectedTableCellModel = (nextCommand: TextEditorCommand) => {
    ...
    const { doc: nextDoc, modified } = mapSelectedTableCellsInDoc(currentDoc, locations, (blocks) =>
      mapTextInBlocks(blocks, (inline) => applyTextCommandToLeaf(inline, nextCommand, options)));
    if (!modified) {
      console.log("[cell-sel] applyFormatCommandToSelectedTableCellModel: no cells matched (possibly nested table), falling through to DOM approach");
      return false;
    }
  ...
  const tsvToTableHtml = (text: string) => {
    ...
    const isPipeTable = lines.every((line) => /^\s*\|/.test(line));
    let rows: string[][];
    if (isPipeTable) {
      rows = lines
    ...
    return wrapRichTextTableHtml(
      `<colgroup>${Array.from({ length: maxColumns }, () => '<col style="width: 120px;" />').join("")}</colgroup><tbody>${rows.map((cells) => `<tr>${cells.map((cell) => `<td>${wrapTableCellContentHtml(`<div class="text-block text-block-paragraph" data-block-kind="paragraph"><p>${escapeHtml(cell) || "<br />"}</p></div>`)}</td>`).join("")}</tr>`).join("")}</tbody>`,
      { colWidths: Array(maxColumns).fill(120) },
    );
  };
  ...
  const insertNodeLinkAtSelection = (cmd: TextEditorCommand) => {
    ...
    // Restore saved position (path survives text-node normalization; cloneRange as fallback)
    let insertRange: Range | null = restoreCursorPath() ?? restoreSavedSelectionRange();
    ...
    const hasSelection = !insertRange.collapsed;
    const selectedText = hasSelection ? insertRange.toString().trim() : "";
